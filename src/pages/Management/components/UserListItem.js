...
export default function UserListItem(props) {
  ...
  return (
    <tr className="ListRow">
      <td
        className="UserId Cursor"
        onClick={() => {
          props.InfoModalClose();
          props.SelUserNickname(props.nickname);
        }}
      >
        {props.userid}
      </td>
      ...
      <td className="ChangeReliable">
        <input
          className="InputReliable"
          onChange={(e) => SetReliable(e.target.value)}
        />
        <button className="SubmitReliable" onClick={ReliableClick}>
          확인
        </button>
      </td>
      <td className="PermanentBan">
        <button className="BanButton" onClick={BanClick}>
          영정
        </button>
      </td>
    </tr>
