...
export default function QnARead() {
  ...
  if (cookie === "true") {
    ...
    if (managerid !== null) IsManager = true;
  }
  ...
  // 공지사항 정보 불러오기
  useEffect(() => {
    Axios.get("http://localhost:8080/qna/read/" + qid).then((res) => {
    ...
    <div>
      ...
      <main className="qna">
        ...
        <div className="buttons">
          <button className="back" type="button" onClick={ListClick}>
            목록
          </button>
          <button
            className="submit"
            type="button"
            hidden={IsManager ? (IsAnswered ? true : false) : true}
            onClick={AnswerClick}
          >
            답변하기
          </button>
