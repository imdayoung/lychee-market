...
export default function NoticeRead() {
  ...
  // 공지사항 정보 불러오기
  useEffect(() => {
    Axios.get("http://localhost:8080/notice/read/" + NoticeId).then((res) => {
  ...
  const UpdateClick = () => {
    if (window.confirm("해당 내용으로 수정하시겠습니까?") === true) {
      ...
      Axios.post("http://localhost:8080/notice/update", {
        ...
        console.log(res);
        if (res.data === true) alert("공지사항 수정이 완료되었습니다.");
        else alert("공지사항 수정을 실패하였습니다.");
        Navigate("/notice");
      });
    }
  };
  ...
  const DeleteClick = () => {
    if (window.confirm("정말 삭제하시겠습니까?") === true) {
      Axios.post("http://localhost:8080/notice/delete", {
        ...
        console.log(res);
        if (res.data === false) alert("공지사항 삭제를 실패하였습니다.");
        Navigate(-1);
      });
    }
  };
    ...
    <div>
      ...
      <main className="noticeMain">
        <table className="noticeRead">
          <tbody>
            ...
            <tr className="n_row">
              ...
              <td>
                <input
                  className="n_td"
                  type="text"
                  value={IsManager ? UpdateDate : NoticeDate}
                  readOnly
                />
              </td>
            </tr>
            ...
            <tr className="n_row">
              ...
              <td>
                <img
                  className="n_td_image"
                  src={NoticeImg}
                  alt="이미지"
                  hidden={NoticeImg === null ? true : false}
                />
        ...
        <div className="buttons">
          <button className="list" type="button" onClick={ListClick}>
            목록
          </button>
          <button
            className="update"
