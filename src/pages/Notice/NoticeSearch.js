...
export default function NoticeSearch() {
  ...
  // 검색한 공지사항 정보 불러오기
  useEffect(() => {
    Axios.get("http://localhost:8080/notice/search/" + SearchWord).then(
      (res) => {
  ...
  if (Notice.length === 0) {
    NoticeList.push(
      <tr key={0} className="n_list_row">
        <td colSpan="4">"{SearchWord}"에 대한 검색결과가 없습니다.</td>
      </tr>
    );
  } else {
    for (let i = Notice.length - 1; i >= 0; i--) {
      NoticeList.push(
        <NoticeListComponent
          ...
          notice_date={moment(Notice[i].notice_date).format("YYYY-MM-DD")}
    ...
    <div>
      ...
      <main className="noticeMain">
        <div className="SearchResult">
          <span>{SearchWord}</span>에 대한 검색결과입니다.
        </div>
        ...
        <div className="noticeBottom">
          <div className="searchNotice">
            ...
            />
            <Link
              to={{ pathname: "/notice/search/" + Word }}
          ...
          <div>
            <Link to="/notice">
              <button className="allNotice" type="button">
                전체 목록
              </button>
            </Link>
            <Link to="/notice/write">
              <button className="writeNotice" type="button" hidden={!IsManager}>
                공지 작성
              </button>
            </Link>
          </div>
